...
public class Raycast {
	...
	public void setRenderBuffer(ImageBuffer buffer) {
		...
		this.totalBoxes = 2 * coordsX * coordsY;
		this.renderCoords = new int[this.totalBoxes];
		
		for ( int j = 0; j < coordsY; j++ ) {
			for ( int i = 0; i < coordsX; i++ ) {
				this.renderCoords[i * 2 + j * coordsX * 2] = i;
				this.renderCoords[i * 2 + j * coordsX * 2 + 1] = j;
			}
		}
		...
		this.stepX = 1.0f / (this.width - 1);
		this.stepY = 1.0f / (this.height - 1);
	}

	public void setScene(Scene scene) {
		this.scene = scene;
		
		this.samples = scene.getImageData().getSamples();
		this.iSamples = 1.0f / (float) this.samples;
