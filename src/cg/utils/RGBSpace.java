...
public class RGBSpace {
	public static final RGBSpace ADOBE = new RGBSpace(0.6400f, 0.3300f,
			0.2100f, 0.7100f, 0.1500f, 0.0600f, 0.31271f, 0.32902f, 2.2f, 0);
	public static final RGBSpace APPLE = new RGBSpace(0.6250f, 0.3400f,
			0.2800f, 0.5950f, 0.1550f, 0.0700f, 0.31271f, 0.32902f, 1.8f, 0);
	public static final RGBSpace NTSC = new RGBSpace(0.6700f, 0.3300f, 0.2100f,
			0.7100f, 0.1400f, 0.0800f, 0.31010f, 0.31620f, 20.0f / 9.0f, 0.018f);
	public static final RGBSpace HDTV = new RGBSpace(0.6400f, 0.3300f, 0.3000f,
			0.6000f, 0.1500f, 0.0600f, 0.31271f, 0.32902f, 20.0f / 9.0f, 0.018f);
	public static final RGBSpace SRGB = new RGBSpace(0.6400f, 0.3300f, 0.3000f,
			0.6000f, 0.1500f, 0.0600f, 0.31271f, 0.32902f, 2.4f, 0.00304f);
	public static final RGBSpace CIE = new RGBSpace(0.7350f, 0.2650f, 0.2740f,
			0.7170f, 0.1670f, 0.0090f, 1 / 3.0f, 1 / 3.0f, 2.2f, 0);
	public static final RGBSpace EBU = new RGBSpace(0.6400f, 0.3300f, 0.2900f,
			0.6000f, 0.1500f, 0.0600f, 0.31271f, 0.32902f, 20.0f / 9.0f, 0.018f);
	public static final RGBSpace SMPTE_C = new RGBSpace(0.6300f, 0.3400f,
			0.3100f, 0.5950f, 0.1550f, 0.0700f, 0.31271f, 0.32902f,
			20.0f / 9.0f, 0.018f);
	public static final RGBSpace SMPTE_240M = new RGBSpace(0.6300f, 0.3400f,
			0.3100f, 0.5950f, 0.1550f, 0.0700f, 0.31271f, 0.32902f,
			20.0f / 9.0f, 0.018f);
	public static final RGBSpace WIDE_GAMUT = new RGBSpace(0.7347f, 0.2653f,
			0.1152f, 0.8264f, 0.1566f, 0.0177f, 0.3457f, 0.3585f, 2.2f, 0);

	private final float gamma, breakPoint, iGamma;
	...
	private RGBSpace(float xRed, float yRed, float xGreen, float yGreen,
			float xBlue, float yBlue, float xWhite, float yWhite, float gamma,
			float breakPoint) {
		this.gamma = gamma;
		iGamma = 1.0f / gamma;
		...
		if (breakPoint > 0.0f) {
			slope = 1.0f / (gamma
					/ (float) Math.pow(breakPoint, 1.0f / gamma - 1.0f) - gamma
					* breakPoint + breakPoint);
			slopeMatch = gamma * slope
					/ (float) Math.pow(breakPoint, 1.0f / gamma - 1.0f);
			segmentOffset = slopeMatch
					* (float) Math.pow(breakPoint, 1.0f / gamma) - slope
					* breakPoint;
		} else {
			slope = 1.0f;
			slopeMatch = 1.0f;
		...
		for (int i = 0; i < 256; i++) {
			float c = i / 255.0f;
			GAMMA_CURVE[i] = clamp((int) (gammaCorrect(c) * 255 + 0.5f), 0, 255);
			INV_GAMMA_CURVE[i] = clamp((int) (ungammaCorrect(c) * 255 + 0.5f),
					0, 255);
		}
		...
		float s = 1.0f / (this.rx * (this.gy * this.bz - this.by * this.gz)
				- this.ry * (this.gx * this.bz - this.bx * this.gz) + this.rz
				* (this.gx * this.by - this.bx * this.gy));
	...
	// return convertXYZtoRGB(c.x, c.y, c.z);
	// }
	...
	public final float gammaCorrect(final float v) {
		if (v <= 0.0f) {
			return 0.0f;
		} else if (v >= 1.0f) {
			return 1.0f;
			...
			return slopeMatch * (float) Math.pow(v, iGamma) - segmentOffset;
		}
	}
