...
public class RGBSpace {
    ...
    public RGBSpace(float xRed, float yRed, float xGreen, float yGreen, float xBlue, float yBlue, float xWhite, float yWhite, float gamma, float breakPoint) {
        ...
        if (breakPoint > 0.0f) {
            slope = 1.0f / (gamma / (float) Math.pow(breakPoint, 1.0f / gamma - 1.0f) - gamma * breakPoint + breakPoint);
            slopeMatch = gamma * slope / (float) Math.pow(breakPoint, 1.0f / gamma - 1.0f);
            segmentOffset = slopeMatch * (float) Math.pow(breakPoint, 1.0f / gamma) - slope * breakPoint;
        } else {
            slope = 1.0f;
            slopeMatch = 1.0f;
            segmentOffset = 0.0f;
        ...
        for (int i = 0; i < 256; i++) {
            ...
            INV_GAMMA_CURVE[i] = clamp((int) (ungammaCorrect(c) * 255 + 0.5f), 0, 255);
        }
        ...
        float s = 1.0f / (this.rx * (this.gy * this.bz - this.by * this.gz) - this.ry * (this.gx * this.bz - this.bx * this.gz) + this.rz * (this.gx * this.by - this.bx * this.gy));
...
//    }
    ...
    public final float gammaCorrect(float v) {
        if (v <= 0.0f)
            ...
            return slopeMatch * (float) Math.pow(v, 1.0f / gamma) - segmentOffset;
    }
